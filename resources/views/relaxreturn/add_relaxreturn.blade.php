                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="container">
                                ...
                                <form action="{{ route('store.relaxreturn') }}" method="POST">
                                    @csrf
                                    <!-- Purchase Request Fields -->

                                    <div class="text-center">
                                        <h4>ADD RELAX RETURN</h4>
                                    </div>
                                    <hr>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group row">
                                                <label for="allocation" class="col-sm-3 col-form-label">FROM</label>
                                                <div class="col-sm-9">
                                                    <select class="form-select form-select-sm" style="width: 100%;"
                                                        id="allocation" name="allocation">

                                                        <option value="RETURN SUPPLIER">CUTTING</option>
                                                        <option value="RETURN SUPPLIER">OTHER</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <a href="{{ route('all.relaxout') }}" class="btn btn-danger mt-2">Back</a>
                                </form>
                            </div>
                ...
                if (event.keyCode === 13) { // 13 is the code for the Enter key
                    ...
                    if (originalNo) {
                        if (checkDuplicate(originalNo, row)) {
                            ...
                            $.ajax({
                                url: '/get/relaxinoriginal/' + originalNo,
                                type: 'GET',
                                success: function(response) {
                                    ...
                                    if (response.material_in_detail) {
                                        ...
                                        console.log(response.material_in_detail);

                                        row.find('input[name$="[receive_date]"]').val(response
                                            .material_in_detail.q_r_code.received_date || ''
                                        );
                                        row.find('input[name$="[supplier_name]"]').val(response
                                            .material_in_detail.q_r_code.supplier_name || ''
                                        );
                                        row.find('input[name$="[item_code]"]').val(response
                                            .material_in_detail.item_code || '');
                                        row.find('input[name$="[po]"]').val(response
                                            .material_in_detail.q_r_code.po || '');
                                        row.find('input[name$="[color_code]"]').val(response
                                            .material_in_detail.color_code || '');
                                        row.find('input[name$="[color_name]"]').val(response
                                            .material_in_detail.color_name || '');
                                        row.find('input[name$="[batch]"]').val(response
                                            .material_in_detail.q_r_code.batch || '');
                                        row.find('input[name$="[roll]"]').val(response
                                            .material_in_detail.q_r_code.roll || '');
                                        row.find('input[name$="[gross_weight]"]').val(response
                                            .material_in_detail.q_r_code.gross_weight || '');
                                        row.find('input[name$="[net_weight]"]').val(response
                                            .material_in_detail.q_r_code.net_weight || '');
                                        // row.find('input[name$="[qty]"]').val(response
                                        //     .material_in_detail.qty || '');
                                        row.find('input[name$="[basic_width]"]').val(response
                                            .material_in_detail.q_r_code.basic_width || '');
                                        row.find('input[name$="[basic_grm]"]').val(response
                                            .material_in_detail.q_r_code.basic_grm || '');
                                        row.find('input[name$="[mo]"]').val(response
                                            .material_in_detail.mo || '');
                                        // row.find('input[name$="[actual_weight]"]').val(response
                                        //     .material_in_detail.actual_weight || '');
